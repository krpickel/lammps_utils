...
from objects.ClusterDeleted import ClusterDeleted
from objects.DeletedClusterMassPercentOverTime import DeletedClusterMassPercentOverTime
        ...
        if (int(timestep) <= 2900000):
            for cluster in delTimesteps[timestep]:
                if(cluster.clusterStr == clusterName):
                    deletedClusterMassPercentTimestep = DeletedClusterMassPercentOverTime()
                    deletedClusterWeightTotals[clusterName] += cluster.getTotalWeight()
                    deletedClusterMassPercentTimestep.timestep = timestep
                    deletedClusterMassPercentTimestep.totalMassPercent = 100*(deletedClusterWeightTotals[clusterName] / initial_mass)
                    deletedClusterMassPercentageOverTime[clusterName].append(deletedClusterMassPercentTimestep)
        
    deletedClusterWeightTotals[clusterName] = round(deletedClusterWeightTotals[clusterName], 2)
    deletedClusterMassPercentage[clusterName] = round(100 * deletedClusterWeightTotals[clusterName] / initial_mass, 2)
            
    tableRow = {nameColStr: clusterName, totalMassStr: deletedClusterWeightTotals[clusterName],
