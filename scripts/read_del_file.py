        ...
        if(inClusterStr.__contains__(hydroStr)):
            if(inClusterStr.__len__() > inClusterStr.index(inClusterStr) + 1):
                if(inClusterStr[inClusterStr.index(inClusterStr) + 1].isdigit()):
                    self.numHydrogen = int(inClusterStr[inClusterStr.index(inClusterStr) + 1])

        if(inClusterStr.__contains__(carbonStr)):
            print(inClusterStr.__len__())
            if(inClusterStr.__len__() > inClusterStr.index(inClusterStr) + 1):
                if(inClusterStr[inClusterStr.index(inClusterStr) + 1].isdigit()):
                    self.numCarbon = int(inClusterStr[inClusterStr.index(inClusterStr) + 1])
        
        if(inClusterStr.__contains__(oxygenStr)):
            print(inClusterStr.__len__())
            if(inClusterStr.__len__() > inClusterStr.index(inClusterStr) + 1):
                if(inClusterStr[inClusterStr.index(inClusterStr) + 1].isdigit()):
                    self.numOxygen = int(inClusterStr[inClusterStr.index(inClusterStr) + 1])
...
outDir = 'C:/Users/asmon/mol_dyn/research/Phenolic_Resin/TP_Reax/4k/H2O/pyro/rep1/analysis'
...
for file in logFiles:
    ...
    with open(inDir + '/' + file, 'r') as file:
        for line in file:
            atomDeleted = atomDeleted.__init__()
            atomsDeleted = []
            data = line.split()
